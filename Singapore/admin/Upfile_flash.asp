...
if upload.form("act")="uploadfile" then
 ...
 if filelx="rar" then
	if fileext<>"rar" and fileext<>"doc" And fileext<>"xls"  then
 ...
 if filelx="swf" then
	if fileext<>"swf"  and fileext<>"flv" then
		response.write "<span style=""font-family: 宋体; font-size: 9pt"">只能上传.Swf和.flv文件！　[ <a href=# onclick=history.go(-1)>重新上传</a> ]</span>"
		response.end
	end if
 end if
