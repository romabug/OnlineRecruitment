...
<table width="950" border="0" cellspacing="0" cellpadding="4">
        <tr> 
		  ...
		  <% 
		  If PageNo > 1 Then
		   Response.write "<a href=""?PageNo=1&slt_subcategory="&JobClass&"&slt_workplacecity="&Area&"&Industry="&Industry&"&PublishDate="&PublishDate&"&Key="&Key&"&KeyClass="&KeyClass&""">首页</a>&nbsp;"
		   Response.write "<a href=""?PageNo="&PageNo-1&"&slt_subcategory="&JobClass&"&slt_workplacecity="&Area&"&Industry="&Industry&"&PublishDate="&PublishDate&"&Key="&Key&"&KeyClass="&KeyClass&""">上页</a>&nbsp;"    
		  Else
		   Response.write "首页&nbsp;"
		   Response.write "上页&nbsp;" 
		  End if
           ...
           <select name="menu1" onChange="MM_jumpMenu('parent',this,0)" style="width:40px;">
		    <% For i = 1 to PCount %>
                <option value="?PageNo=<%= i %>&slt_subcategory=<%= JobClass %>&slt_workplacecity=<%= Area %>&Industry=<%= Industry %>&PublishDate=<%= PublishDate %>&Key=<%= Key %>&KeyClass=<%= KeyClass %>" <% If i = PageNo Then Response.write "Selected"%>><%=i%></option>
		    <% Next %>
           </select>
            <% 
		  If PageNo < PCount Then
		   Response.write "<a href=""?PageNo="&PageNo+1&"&slt_subcategory="&JobClass&"&slt_workplacecity="&Area&"&Industry="&Industry&"&PublishDate="&PublishDate&"&Key="&Key&"&KeyClass="&KeyClass&""">下页</a>&nbsp;"
		   Response.write "<a href=""?PageNo="&PCount&"&slt_subcategory="&JobClass&"&slt_workplacecity="&Area&"&Industry="&Industry&"&PublishDate="&PublishDate&"&Key="&Key&"&KeyClass="&KeyClass&""">尾页</a>"   
		  Else
		   Response.write "下页&nbsp;"
		   Response.write "尾页" 
		  End if
