...
uid=request.QueryString("uid")
  ...
  if request.Form("chkNO")="" then
  response.Write("<script language=javascript>alert('请选择人才！');javascript:history.go(-1);</script>")
  response.End()
  else
  uid=request.Form("chkNO")
  end if
end if
cid=request.cookies("01387job")("cid")
set rs=server.createobject("adodb.recordset")
rs.open "select * from [01387company] where username='"&request.cookies("01387job")("companyusername")&"' and cid='"&cid&"' order by id desc" ,conn,1,1
    ...
    <td valign="top" style="BORDER-left: #9DB3C5 1px solid;"><!--#include file="welcome.asp"-->
	  ...
	  response.Write("7000以上")
        ...
        <table width="700" border="0" align="center" cellpadding="0" cellspacing="0">
          ...
          <tr>
            <td height="80" align="center"><input type="button" value="返回人才列表" name="Button" style="width:120;height:26" onClick="history.back(-1)" />
                <input name="Button2" type="button" id="Button2" style="width:120;height:26" onClick="location.href='Apply_InterviewSelf.asp?from=ShowCheckPerson&uid=<%=rs1("uid")%>&amp;page=<%=currentpage%>&amp;MyChkgo=<%=MyChkgo%>'" value="发送面试邀请" /></td>
          </tr>
        </table></td>
  </tr>
</table>
