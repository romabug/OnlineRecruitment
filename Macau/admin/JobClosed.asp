...
Call RsName(rs,"Select jobname,closed,closed_reason,closed_time,closed_overtime from [01387joblist] where jid='"&ChkNo&"'",1,1)
if rs(1)=true then
	response.write("<script language=javascript>if (confirm('该职位已冻结，是否对其解冻？')){")
	response.write("location.href='JobOpened.asp?ChkNo="&ChkNo&"&url="&request.ServerVariables("HTTP_REFERER")&"';")
	response.write("}else{")
...
<table width="99%" border="0" align="center" cellpadding="3" cellspacing="1" bgcolor="#FFFFFF" class="table">
  <form name="form1" method="post" action="JobClosedGo.asp?ChkNo=<%=chkNo%>&url=<%=request.ServerVariables("HTTP_REFERER")%>">
    <tr> 
      <td height="25" colspan="2" background="image/admin_bg_1.gif"> <div align="center"><strong><font color="#FFFFFF">冻 
          结 职 位</font></strong></div></td>
    </tr>
    <tr bgcolor="#f1f3f5"> 
      <td height="28"><div align="right">职位：</div></td>
    ...
    <tr> 
      <td height="26" colspan="2" bgcolor="#E6E9EC"> <div align="center"> 
          ...
          <input type="button" name="btnPower2" value="返 回" onClick="location.href='<%=request.ServerVariables("HTTP_REFERER")%>'" style="font-size=12;height=26">
        </div></td>
    </tr>
  </form>
</table>
